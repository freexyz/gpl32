...
void gc0308_init(short nWidthH, short nWidthV,	unsigned short uFlag)
{
	...
	sccb_wr_r8d8(GC0308_ID, 0xe6, 0x03);	//exp level 2
	sccb_wr_r8d8(GC0308_ID, 0xe7, 0x84);
	sccb_wr_r8d8(GC0308_ID, 0xe8, 0x07);	//exp level 3
	sccb_wr_r8d8(GC0308_ID, 0xe9, 0x08);
	sccb_wr_r8d8(GC0308_ID, 0xea, 0x0d);	//exp level 4
	sccb_wr_r8d8(GC0308_ID, 0xeb, 0x7a);
	...
	sccb_wr_r8d8(GC0308_ID, 0x11, 0x0d);	//fd//[7:4]tx_width, [3:0]space width,*2
	sccb_wr_r8d8(GC0308_ID, 0x12, 0x2a);	//sh_delay
	sccb_wr_r8d8(GC0308_ID, 0x13, 0x00);	//[3:0] row_tail_width
	sccb_wr_r8d8(GC0308_ID, 0x14, 0x10);	//[7]hsync_always ,[6] NA,  [5:4] CFA sequence
	...
	sccb_wr_r8d8(GC0308_ID, 0x1a, 0x2a);	//1e//[7]rsv1,[6]rsv0, [5:4]coln_r,
	sccb_wr_r8d8(GC0308_ID, 0x1b, 0x00);	//[7:2]NA, [1:0]BIN4 AND BIN2
	sccb_wr_r8d8(GC0308_ID, 0x1c, 0x49);	//c1//[7]hrst_enbale, [6:4]da_rsg, [3]tx high enable, [2]NA, [1:0]da18_r
	sccb_wr_r8d8(GC0308_ID, 0x1d, 0x9a);	//08//[7]vref_en, [6:4]da_vef, [3]da25_en, [2]NA, [1:0]da25_r,set da25 voltage
	sccb_wr_r8d8(GC0308_ID, 0x1e, 0x61);	//60//[7]LP_MTD,[6:5]opa_r,ADC's operating current,  [4:2]NA, [1:0]sref
	...
	sccb_wr_r8d8(GC0308_ID, 0x3a, 0x20);	//{7:6}offset submode {5:0}offset ratio
	...
	sccb_wr_r8d8(GC0308_ID, 0x50, 0x14);	//10  //global gain
	sccb_wr_r8d8(GC0308_ID, 0x53, 0x80);	//G
	sccb_wr_r8d8(GC0308_ID, 0x54, 0x87);	//R channel gain
	sccb_wr_r8d8(GC0308_ID, 0x55, 0x87);	//B channel gain
	sccb_wr_r8d8(GC0308_ID, 0x56, 0x80);
	sccb_wr_r8d8(GC0308_ID, 0x8b, 0x10);	//r2
	sccb_wr_r8d8(GC0308_ID, 0x8c, 0x10);	//g2
	sccb_wr_r8d8(GC0308_ID, 0x8d, 0x10);	//b2
	sccb_wr_r8d8(GC0308_ID, 0x8e, 0x10);	//r4
	sccb_wr_r8d8(GC0308_ID, 0x8f, 0x10);	//g4
	sccb_wr_r8d8(GC0308_ID, 0x90, 0x10);	//b4
	...
	sccb_wr_r8d8(GC0308_ID, 0x77, 0x47);	//[7:4]edge1_effect [3:0]edge2_effect
	...
	sccb_wr_r8d8(GC0308_ID, 0x93, 0x46);	// <--40
	sccb_wr_r8d8(GC0308_ID, 0x94, 0x00);
	sccb_wr_r8d8(GC0308_ID, 0x95, 0x03);
	sccb_wr_r8d8(GC0308_ID, 0x96, 0xd0);
	sccb_wr_r8d8(GC0308_ID, 0x97, 0x40);	//
	sccb_wr_r8d8(GC0308_ID, 0x98, 0xf0);
	sccb_wr_r8d8(GC0308_ID, 0xb1, 0x3c);	//manual cb
	sccb_wr_r8d8(GC0308_ID, 0xb2, 0x3c);	//manual cr
	sccb_wr_r8d8(GC0308_ID, 0xb3, 0x40);
	sccb_wr_r8d8(GC0308_ID, 0xb6, 0xe0);
	sccb_wr_r8d8(GC0308_ID, 0xbd, 0x3C);
	...
	sccb_wr_r8d8(GC0308_ID, 0xd3, 0x88);	//Y_target and low pixel thd high X4 low X2
	sccb_wr_r8d8(GC0308_ID, 0xd5, 0xF2);	//lhig
	sccb_wr_r8d8(GC0308_ID, 0xd6, 0x10);	// ignore mode
	...
	sccb_wr_r8d8(GC0308_ID, 0xc1, 0x14);	//Y_gamma_1
	sccb_wr_r8d8(GC0308_ID, 0xc2, 0x21);	//Y_gamma_2
	sccb_wr_r8d8(GC0308_ID, 0xc3, 0x36);	//Y_gamma_3
	sccb_wr_r8d8(GC0308_ID, 0xc4, 0x49);	//Y_gamma_4
	sccb_wr_r8d8(GC0308_ID, 0xc5, 0x5B);	//Y_gamma_5
	sccb_wr_r8d8(GC0308_ID, 0xc6, 0x6B);	//Y_gamma_6
	sccb_wr_r8d8(GC0308_ID, 0xc7, 0x7B);	//Y_gamma_7
	sccb_wr_r8d8(GC0308_ID, 0xc8, 0x98);	//Y_gamma_8
	sccb_wr_r8d8(GC0308_ID, 0xc9, 0xB4);	//Y_gamma_9
	sccb_wr_r8d8(GC0308_ID, 0xca, 0xCE);	//Y_gamma_10
	sccb_wr_r8d8(GC0308_ID, 0xcb, 0xE8);	//Y_gamma_11
	sccb_wr_r8d8(GC0308_ID, 0xcc, 0xFF);	//Y_gamma_12
	sccb_wr_r8d8(GC0308_ID, 0xf0, 0x02);
	sccb_wr_r8d8(GC0308_ID, 0xf1, 0x01);
	sccb_wr_r8d8(GC0308_ID, 0xf2, 0x04);	//manual stretch K
	sccb_wr_r8d8(GC0308_ID, 0xf3, 0x30);	//the limit of Y_stretch
	sccb_wr_r8d8(GC0308_ID, 0xf9, 0x9f);
	sccb_wr_r8d8(GC0308_ID, 0xfa, 0x78);
	sccb_wr_r8d8(GC0308_ID, 0xfe, 0x01);
	sccb_wr_r8d8(GC0308_ID, 0x00, 0xf5);	//high_low limit
	sccb_wr_r8d8(GC0308_ID, 0x02, 0x20);	//y2c
	sccb_wr_r8d8(GC0308_ID, 0x04, 0x10);
	sccb_wr_r8d8(GC0308_ID, 0x05, 0x10);
	sccb_wr_r8d8(GC0308_ID, 0x06, 0x20);
	sccb_wr_r8d8(GC0308_ID, 0x08, 0x15);
	sccb_wr_r8d8(GC0308_ID, 0x0a, 0xa0);	// number limit
	sccb_wr_r8d8(GC0308_ID, 0x0b, 0x64);	// skip_mode
	...
	sccb_wr_r8d8(GC0308_ID, 0x11, 0x37);	  // 0x3f
	sccb_wr_r8d8(GC0308_ID, 0x12, 0x24);
	sccb_wr_r8d8(GC0308_ID, 0x13, 0x39);	//13//smooth 2
	sccb_wr_r8d8(GC0308_ID, 0x14, 0x45);	//R_5k_gain_base
	sccb_wr_r8d8(GC0308_ID, 0x15, 0x45);	//B_5k_gain_base
	...
	sccb_wr_r8d8(GC0308_ID, 0x19, 0x55);	//X2 thd
	sccb_wr_r8d8(GC0308_ID, 0x1a, 0xd8);	//e4//d0//Y1 thd
