...
void encode_block_demo(void)
{
	char SD_ID[24] = "c:\\chen52.jpg";// Fetch file on SD Card
	char SD_ID_1[24] = "c:\\chen53.jpg";// Fetch file on SD Card
         ...
         {      
           case FS_SD:
                if(encode_info.EncodeMode==IMAGE_ENCODE_ONCE_READ)
                    sprintf((char *)path, (const char *)"c:\\readonce%03d.jpeg", pic_num);
                else if(encode_info.EncodeMode==IMAGE_ENCODE_BLOCK_READ)   
                    sprintf((char *)path, (const char *)"c:\\blockread%03d.jpeg", pic_num);
                else
           ...
           case FS_NAND1:
                 if(encode_info.EncodeMode==IMAGE_ENCODE_ONCE_READ)
                    sprintf(path, (const char *)"a:\\readonce%03d.jpeg", pic_num);
                else if(encode_info.EncodeMode==IMAGE_ENCODE_BLOCK_READ)   
                    sprintf((char *)path, (const char *)"a:\\blockread%03d.jpeg", pic_num);
                else
           ...
           case FS_USBH:
                 if(encode_info.EncodeMode==IMAGE_ENCODE_ONCE_READ)
                    sprintf((char *)path, (const char *)"G:\\readonce%03d.jpeg", pic_num);
                else if(encode_info.EncodeMode==IMAGE_ENCODE_BLOCK_READ)   
                    sprintf((char *)path, (const char *)"G:\\blockread%03d.jpeg", pic_num);
                else
                    sprintf(path, (const char *)"G:\\blockrw%03d.jpeg", pic_num);
           break;
         }
