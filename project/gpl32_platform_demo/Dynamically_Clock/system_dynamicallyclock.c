...
INT32S system_sdram_timing_set(CLK_ENUM system_clk,INT16U tRC,INT16U tRAS,INT16U tRP,INT16U tRCD)__attribute__ ((section(".ap_entry")));
INT32S system_nand_timing_set(CLK_ENUM system_clk,INT16U tRP,INT16U tH,INT16U tWP)__attribute__ ((section(".ap_entry")));
INT32U system_hardware_Divider_set(INT32U Dividend,INT32U Divisor,INT32U Remainder_CK)__attribute__ ((section(".ap_entry")));
	...
	switch(target_clk)
	{
		case CLK_48MHZ:
			...
			{	// speed up -> slow down
			    ...
			    ASM(NOP);
			    ASM(NOP);			   
			    system_sdram_timing_set(CLK_48MHZ,SDRAM_tRC,SDRAM_tRAS,SDRAM_tRP,SDRAM_tRCD);
			    system_nand_timing_set(CLK_48MHZ,Nand_tRP,Nand_tH,Nand_tWP);
			}
		...
		case CLK_63MHZ:
			...
			{	// speed up -> slow down
			    ...
			    ASM(NOP);
			    ASM(NOP);			   
			    system_sdram_timing_set(CLK_63MHZ,SDRAM_tRC,SDRAM_tRAS,SDRAM_tRP,SDRAM_tRCD);
			    system_nand_timing_set(CLK_63MHZ,Nand_tRP,Nand_tH,Nand_tWP);
			}
		...
		case CLK_78MHZ:
			...
			{	// speed up -> slow down
			    ...
			    ASM(NOP);
			    ASM(NOP);			    
			    system_sdram_timing_set(CLK_78MHZ,SDRAM_tRC,SDRAM_tRAS,SDRAM_tRP,SDRAM_tRCD);
			    system_nand_timing_set(CLK_78MHZ,Nand_tRP,Nand_tH,Nand_tWP);
			}
		...
		case CLK_96MHZ:
			...
			{	// slow down -> speed up				
				system_sdram_timing_set(CLK_96MHZ,SDRAM_tRC,SDRAM_tRAS,SDRAM_tRP,SDRAM_tRCD);
				system_nand_timing_set(CLK_96MHZ,Nand_tRP,Nand_tH,Nand_tWP);
				ASM(NOP);
			    ASM(NOP);
			    ASM(NOP);
			    ASM(NOP);
...
INT32S system_sdram_MHZ_set(CLK_ENUM system_clk)
{
    ...
    cbrcyc_reg_val=(Common_Refresh_Period*system_clk/1000)&0xFFFF * oc;
